    ...
    while (*c != '\0') {
        ...
        // Case if readen symbol is '-', '+', '*', '/' or '^' means that we're reading an operation
            ...
            // Binary
                ...
                if (is_commutative(operation(*c, 0)) && expr_op_prty(operation(*c, 0)) + pd < lp){
                ...
                } else if (!(p->ancestor == NULL) && !is_commutative(p->ancestor->op) && expr_op_prty(operation(*c, 0)) + pd == lp) {
                    expr_node* _p;
                    init_empty_expr_node(&_p, _EXPR_OP, operation(*c, 0), NULL);
                    p = p->ancestor;
                    if (! (p->ancestor == NULL)) {
                        p->ancestor->right = _p;
                        _p->ancestor = p->ancestor;
                    }
                    p->ancestor = _p;
                    _p->left = p;
